...
6. Start with all zoning weights zero. This makes the building playable in **SANDBOX → TEST YARD** without changing seeded districts or trying to fit a campus into a house lot. Setting a positive weight deliberately changes district selection; large structures need suitable lot-generation support before doing so. Time Challenge never reads those weights. To appear on selected campaign levels, add opt-in `campaign.levels` as described in [campaign](campaign.md). City Borough / City Downtown also read `campaign.family` and optional `campaign.exception` for composition caps.
7. The eighteen urban mid-rise and skyscraper variants are listed in [urban-variant-roster](urban-variant-roster.md). `scripts/author-urban-variants.mjs` is an opt-in helper that writes those owned packages; production never runs it. Do not point two building IDs at the same layout file.
8. Run `npm test` and `npm run build`. Open the yard, search its ID or traits, jump to it, use **View whole example**, destroy it and restore its bay. Driving resumes the close follow camera. Use the existing Debug roof/floor/reveal controls to inspect rooms.
