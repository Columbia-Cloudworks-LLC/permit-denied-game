...
The previous Vitest worker-source rewrite (`scripts/vitest-rpc-timeout*.mjs`) is gone. Soak and bench yield with `pumpVitestRpc()` so worker ACKs can flush without patching Vitest.
