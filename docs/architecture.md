...
`src/world/dressing.ts` templates (rural residence, family yard, farmstead, roadside service, contractor yard, utility lot, small commercial) place assets relative to lot + building, test occupancy, and keep a driveway corridor. After a building is placed, `src/world/lotUse.ts` assigns the lot profile from building traits: fuel equipment needs a service use, crop fields need an agricultural use, houses stay residential, civic/commercial buildings take the shop profile, and industrial yards take contractor or utility. Same seed reproduces selection, variants, headings, and ground patches.
