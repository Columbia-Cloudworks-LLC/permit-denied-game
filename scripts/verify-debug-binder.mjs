...
const snapshot = () => page.evaluate(() => {
  const inspect = window.__pd.inspect();
  return { ...inspect, scroll: document.querySelector('.debug-pages').scrollTop };
});
...
try {
  await page.goto(base + '/?sandbox=1&seed=81&debug=1'); await page.waitForFunction(() => window.__pd?.version === 1);
  ...
  assert.equal(await page.evaluate(() => window.__pd.townSnapshot().vehicles[0]?.autonomous), true);
  ...
  await page.evaluate(() => { const pose = window.__pd.dozerSnapshot(); window.__pd.setDozerPose({ x: pose.x + 1 }); });
  ...
  for (const id of ['vehicle:bus', 'vehicle:combine', 'vehicle:tractor-trailer', 'building:union-tower', 'site:farmstead', 'fixture:interior-sofa', 'prop:barricade']) {
    ...
    assert.equal((await snapshot()).bays, 1);
    assert.equal(await page.evaluate(() => window.__pd.inspect().yardIssues), 0);
    await page.locator('[data-frame]').click();
  ...
  await page.goto(base + '/?testAsset=missing'); await page.waitForFunction(() => window.__pd?.version === 1);
  ...
  for (const [width, height] of [[390, 844], [844, 390], [360, 640], [640, 360]]) {
    ...
    assert.equal(await mobile.evaluate(() => window.__pd.inspect().mode), 'play');
    ...
    const savedScroll = await mobile.evaluate(() => window.__pd.inspect().binder.scroll.assets);
    ...
    assert.ok(await mobile.evaluate(() => window.__pd.inputAxis().throttle > 0));
