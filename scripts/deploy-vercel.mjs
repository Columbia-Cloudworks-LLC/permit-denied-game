...
export async function staticDeploymentFiles(directory) {
  const files = [{ file: '.vercel/output/config.json', data: Buffer.from(JSON.stringify({
    ...
    routes: [
      ...
      { src: '/sw.js', headers: { 'cache-control': 'no-cache', 'content-type': 'application/javascript; charset=utf-8' }, continue: true },
      { src: '/workbox-(.*)\\.js', headers: { 'cache-control': 'public, max-age=31536000, immutable' }, continue: true },
      { src: '/manifest.webmanifest', headers: { 'cache-control': 'no-cache', 'content-type': 'application/manifest+json' }, continue: true },
      { handle: 'filesystem' },
